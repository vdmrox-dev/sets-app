import { retractableSessions } from "./sessionLog";
...
export { isValidPlanShape } from "./parsePlan";
...
// Removes a logged Session if it is among the five most recent. Older
// Sessions are permanent. Returns the remaining sessions. Bails on a missing
// id rather than matching every session that also lacks one.
export function deleteSession(sessionId) {
  if (!sessionId) return getSessions();
  const sessions = getSessions();
  const allowed = new Set(retractableSessions(sessions).map((s) => s.id));
  if (!allowed.has(sessionId)) return sessions;
